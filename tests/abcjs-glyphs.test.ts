...
import { ABCJS_GLYPHS, ABCJS_STAFF_SPACE } from '../src/renderer/glyphs-abcjs.js'
...
describe('the generated abcjs glyph table', () => {
  ...
  it('reproduces the outlines abcjs actually drew, verbatim', () => {
    ...
    const heads = drawnPaths(golden('simple-c')).filter((p) => p.cls.includes('notehead'))
  ...
  it('records the staff space its units are expressed in', () => {
    ...
    const width = (ABCJS_GLYPHS['noteheads.quarter']?.w ?? 0) / ABCJS_STAFF_SPACE
