...
function compare(got: number[], want: number[], label: string): Diff | null {
  if (got.length !== want.length) {
    return {
      matched: 0,
      where: `${label}: ${got.length} rows vs ${want.length}`,
    };
  }
  for (const [i, w] of want.entries()) {
    if (got[i] !== w)
      return { matched: i, where: `${label}: row ${i} ${got[i]} vs ${w}` };
  }
  return null;
}
...
function run(c: Case): Diff | null {
  ...
  if (c.kind === "switch") {
    const a = compare(
      timings(c.abc).map((t) => t.milliseconds),
      c.ms ?? [],
      "tune 1",
    );
    if (a !== null) return a;
  ...
  if (perMeasure !== c.millisecondsPerMeasure) {
    return {
      matched: 0,
      where: `millisecondsPerMeasure ${perMeasure} vs ${c.millisecondsPerMeasure}`,
    };
  }
  return compare(
    rows.map((t) => t.milliseconds),
    c.ms ?? [],
    "ms",
  );
}
  ...
  it("writes the ranked table", () => {
    const rows = CASES.map((c) => {
      ...
      return {
        slug: c.slug,
        diff,
        n: (c.ms?.length ?? 0) + (c.ms2?.length ?? 0),
      };
    });
