...
function firstDifference(
  ...
  if (want === null) {
    return got === null
      ? null
      : { matched: 0, where: `expected NO grid, got ${got.length} sections` };
  }
  if (got === null)
    return {
      matched: 0,
      where: `expected ${want.length} sections, got NO grid`,
    };
  ...
  for (const [s, wantSection] of want.entries()) {
    ...
    if (gotSection.type !== wantSection.type) {
      return {
        matched,
        where: `sec ${s} type ${gotSection.type} vs ${wantSection.type}`,
      };
    }
    ...
    if (gotPart.name !== wantSection.name) {
      return {
        matched,
        where: `sec ${s} part name "${gotPart.name}" vs "${wantSection.name}"`,
      };
    }
        ...
        c.expected === null
          ...
          : c.expected.reduce(
              (n, s) =>
                n +
                (s.type === "part"
                  ? s.lines.reduce((k, l) => k + l.length, 0)
                  : 0),
              0,
            );
