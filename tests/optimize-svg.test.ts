...
const hooks = (svg: string): string[] =>
  absolutePixels(svg)
    .items.map(
      (item) => `${item.cls}@${item.x.toFixed(2)},${item.y.toFixed(2)}`,
    )
    .sort();
...
describe("optimizeSVG — <defs>/<use> deduplication", () => {
  ...
  describe("it actually saves", () => {
    it("cuts every glyph-dense fixture by a large fraction", () => {
      ...
      // and unchanged in kind; only the ratio moved, 0.45 → 0.52, 0.65 → 0.75 and 0.60 → 0.72, re-measured once at the end of the
      // markup work rather than nudged at each step. A closed `<path>` for every rule
      // instead of a `<rect>` moved them again, for the same reason: more bytes both sides.
      //
      ...
      // renderer used to draw a clef, key and meter for EVERY voice — hidden duplicates
      // stacked exactly on top of each other. abcjs hangs the staff-extras off a staff's
      // FIRST voice only, and now so do we, so there is less redundancy left to dedupe.
      // The plain output shrank; the optimizer did not.
      for (const [name, limit] of [
        ["ave-verum-corpus", 0.52],
        ["ragtime-mini", 0.75],
        ["zocharti-loch", 0.72],
      ] as const) {
        const abc = fixture(name);
