 ...
 */
const PASSING: readonly string[] = ['midi-drums', 'midi-piano', 'midi-staccato']
...
function bytes(data: string): string[] {
  ...
  const out: string[] = []
  for (let i = 0; i + 2 < body.length + 1; i += 3) out.push(body.substring(i + 1, i + 3).toLowerCase())
  return out
}
...
function firstDifference(got: string, want: string): Diff | null {
  ...
  if (!got.startsWith(PREFIX)) return { matched: 0, where: `no \`${PREFIX}\` prefix` }
  const a = bytes(got)
  const b = bytes(want)
...
function run(c: Case): Diff | null {
  ...
  return firstDifference(midiFile(score, pan === undefined ? {} : { pan }), c.expected)
}
...
describe('MIDI file vs abcjs', () => {
  ...
  it('the gate can tell two different files apart', () => {
    ...
    if (piano === undefined || drums === undefined) throw new Error('missing case')
