...
/**
 * **CLOSED, AND THE NOTE THAT KEPT IT OPEN WAS STALE.** `escaped-percent` was excluded on
 * 2026-08-08 under a paragraph reasoning that the fixture came from abcjs 6.7.0 while our
 * strict target was 6.6.3, where a `%` truncates a line unconditionally — so "one note is
 * the right answer for our target and four is the right answer for a target we have not
 * adopted". **The target moved to 6.7.0 the same day and the note did not**, so a fixture
 * whose entire purpose is that rule sat excluded for a week with every note after the
