...
/**
 * FIXTURES WHOSE GOLDEN WAS NOT GENERATED BY THE abcjs WE ARE PINNED TO.
 *
 * This is not a known divergence and it is not a tolerance — it is a statement about the
 * ORACLE. `abcjs-strict` is defined against **abcjs 6.6.3** (see `CLAUDE.md`), and the
 * vendored copy at `../abcMusicKit/Docs/References/abcjs/abcjs-6.6.3` is unmodified —
 * diffed file by file against a copy taken before this session, only the files carrying my
 * own probe `console.log`s differ.
 *
 * `escaped-percent` arrived in the sibling corpus on 2026-08-08 with a golden that
 * disagrees with that source. Measured three ways on the SAME fixture:
 *
 *     vendored abcjs 6.6.3, rendered   1 notehead,  title "100"
 *     the new golden                   4 notes,     title "100<ZWSP>％ Amazing"
 *     abcts, after the `%` fix         1 note,      title "100\"
 *
 * The fixture's own header says why: it is harvested from abcjs **6.7.0**, where `\%` is a
 * literal percent. Under 6.6.3 a `%` truncates the line unconditionally
 * (`abc_parse.js:408-411`) — no escape, no quoting — which is why one note is the right
 * answer for our target and four is the right answer for a target we have not adopted.
 *
 * SO THIS IS A DECISION, NOT A DEFECT: either the strict target moves to 6.7.0 or the
 * fixture does not belong in a 6.6.3 corpus. Recorded here so the gate states the reason
 * out loud instead of either hiding a red or asserting a number from the wrong engine.
 * Delete the entry the moment the golden is regenerated from the pinned source.
 */
const GOLDEN_FROM_ANOTHER_ABCJS: readonly string[] = ["escaped-percent"];
 ...
 */
const offsetWithin = (
  ourStart: number | undefined,
  start: number,
  end: number,
): boolean => ourStart !== undefined && ourStart >= start && ourStart < end;
        ...
        .map((event) => ({
            ...
            soundingRatio:
              ratToNumber(event.notatedDuration) === 0
                ...
                : ratToNumber(event.duration) /
                  ratToNumber(event.notatedDuration),
            pitches: (event.type === "chord"
              ? event.pitches
              : [event.pitch]
            ).map((pitch) => `${diatonic(pitch)}`),
          ...
          key: keyOf({
            ...
            pitches: (event.type === "chord"
              ? event.pitches
              : [event.pitch]
            ).map((pitch) => `${diatonic(pitch)}${ourAccidental(pitch)}`),
            decorationCount: event.decorations.length,
            hasChordSymbol: event.chordSymbol !== null,
            graceCount: event.graceNotes.length,
          }),
...
const goldenAttachments = (element: GoldenElement) => ({
  ...
  hasChordSymbol:
    element.chord?.some(
      (c) => c.position === "default" && c.rel_position === undefined,
    ) ?? false,
...
it("content parity against abcjs goldens does not regress", () => {
  ...
  // ONLY THE FIXTURES THAT HAVE A GOLDEN. The corpus lives in a sibling repo another agent
  // maintains, and a fixture can land there before its goldens do — four did, mid-session
  // on 2026-08-08. A fixture with no `.parse.json` cannot be compared against abcjs at all,
  // so it is skipped and NAMED in the report rather than crashing the gate on `ENOENT`.
  // "We compared everything" and "we compared everything we had a golden for" are different
  // claims and the second one has to be said out loud.
  const goldens = new Set(readdirSync(goldensDir));
  const fixtures = readdirSync(corpusDir)
    .filter((f) => f.endsWith(".abc"))
    .filter((f) => goldens.has(`${basename(f, ".abc")}.parse.json`))
    .filter((f) => !GOLDEN_FROM_ANOTHER_ABCJS.includes(basename(f, ".abc")))
    .sort();
  const ungoldened = readdirSync(corpusDir)
    .filter((f) => f.endsWith(".abc"))
    .map((f) => basename(f, ".abc"))
    .filter((name) => !goldens.has(`${name}.parse.json`))
    .sort();
  for (const name of ungoldened)
    rows.push(`skip   ${name.padEnd(34)} no abcjs golden yet`);
  for (const name of GOLDEN_FROM_ANOTHER_ABCJS) {
    rows.push(
      `skip   ${name.padEnd(34)} golden is not from the pinned abcjs 6.6.3`,
    );
  }
  ...
  for (const file of fixtures) {
    ...
    const sameContent =
      ...
      ours.every((o, i) =>
        o.microtonal
          ? o.keyNoAccidental === theirs[i]?.keyNoAccidental
          : o.key === theirs[i]?.key,
      );
    const offsetsOk =
      sameContent &&
      ours.filter(
        (o, i) =>
          !offsetWithin(o.start, theirs[i]?.start ?? 0, theirs[i]?.end ?? 0),
  ...
  for (const file of fixtures) {
    ...
    const same =
      ours.length === theirs.length &&
      ours.every((syllable, i) => syllable === theirs[i]);
  ...
  expect(
    lyricsCompared,
    "no fixture had lyrics — the goldens are not loading",
  ).toBeGreaterThan(5);
  expect(
    lyricFailures.sort(),
  ...
  expect(
    compared,
    "no fixtures were comparable — the goldens are not loading",
  ).toBeGreaterThan(0);
  expect(
    beamFailures.sort(),
