...
import { corpusDir, type GoldenElement, goldenElements, goldenNotes, goldensDir } from './corpus.js'
...
it('content parity against abcjs goldens does not regress', () => {
  ...
  const ungoldened = readdirSync(corpusDir)
    ...
    .sort()
  for (const name of ungoldened) rows.push(`skip   ${name.padEnd(34)} no abcjs golden yet`)
  for (const name of GOLDEN_FROM_ANOTHER_ABCJS) {
    rows.push(`skip   ${name.padEnd(34)} golden is not from the pinned abcjs 6.6.3`)
  }
