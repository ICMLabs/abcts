...
const UNMAPPED_SMUFL_REASONS: Readonly<Record<string, string>> = {
  ...
  accidentalThreeQuarterTonesSharpStein:
    "abcjs has no three-quarter-tone glyph",
...
describe("the SMuFL ↔ abcjs glyph bridge", () => {
  ...
  it("maps only names Bravura actually has", () => {
    const missing = Object.keys(SMUFL_TO_ABCJS).filter(
      (name) => !isGlyph(name),
    );
    expect(missing).toEqual([]);
  });
  ...
  it("accounts for every abcjs glyph — mapped or explicitly not", () => {
    const claimed = new Set<string>([
      ...Object.values(SMUFL_TO_ABCJS),
      ...UNMAPPED_ABCJS,
    ]);
    const unaccounted = Object.keys(ABCJS_GLYPHS).filter(
      (name) => !claimed.has(name),
    );
    expect(
      unaccounted,
      "abcjs glyphs neither mapped nor listed as unmapped",
    ).toEqual([]);
  });
  ...
  it("accounts for every Bravura glyph — mapped or explicitly not, WITH A REASON", () => {
    ...
    expect(
      unaccounted,
      "Bravura glyphs with no mapping and no recorded reason",
    ).toEqual([]);
  });
  ...
  it("collapses Above/Below pairs onto one abcjs glyph where abcjs has one", () => {
    ...
    expect(SMUFL_TO_ABCJS["articStaccatoAbove"]).toBe(
      SMUFL_TO_ABCJS["articStaccatoBelow"],
    );
    expect(SMUFL_TO_ABCJS["articTenutoAbove"]).toBe(
      SMUFL_TO_ABCJS["articTenutoBelow"],
    );
    // Marcato is the exception: abcjs DOES ship both orientations.
    expect(SMUFL_TO_ABCJS["articMarcatoAbove"]).not.toBe(
      SMUFL_TO_ABCJS["articMarcatoBelow"],
    );
  });
