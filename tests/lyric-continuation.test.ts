...
import { fontPixels, UNIT_PX } from "../src/renderer/abcjs-constants.js";
import { render } from "../src/renderer/index.js";
import { ENGRAVE, layout } from "../src/renderer/layout.js";

const abc = readFileSync(
  "tests/fixtures/gonzato-4-1-4-directives-as-i-fields.abc",
  "utf-8",
);
...
const drawnLyrics = (mode: CompatibilityMode) => {
  ...
  return layout(score, { mode })
    ...
    .flatMap((element) =>
      element.texts.filter((text) => text.role === "lyric"),
    );
};
...
describe("lyric continuation across interposed directives", () => {
  describe("abcjs-strict — abcjs behaviour preserved, bug included", () => {
    ...
    it("does NOT realize %%vocalfont — abcjs parses it and never draws it", () => {
      ...
      const sizes = new Set(
        drawnLyrics("abcjs-strict").map((text) => text.size),
      );
      expect([...sizes]).toHaveLength(1);
      // 17px, the default `vocalfont` in abcjs's own pixels — expressed through the knob,
      // not through a hard-coded staff space.
      expect(
        drawnLyrics("abcjs-strict").every((text) => text.size === 17 / UNIT_PX),
      ).toBe(true);
      expect(drawnLyrics("abcjs-strict").every((text) => !text.italic)).toBe(
        true,
      );
    });
  });
    ...
    describe(`${mode} — Gonzato §4.1.4 semantics`, () => {
      ...
      it("draws three font runs, per SEGMENT of the one lyric line", () => {
        ...
        const runs = drawn.map(
          (text) =>
            `${text.size.toFixed(3)}${text.bold ? "B" : ""}${text.italic ? "I" : ""}`,
        );
        ...
        // …and the divisor is the LAYOUT UNIT, not a hard-coded staff space: `text.size`
        // is in whatever the layout is denominated in, which is abcjs's own pixels now.
        const size = (points: number) =>
          (fontPixels(points) / UNIT_PX).toFixed(3);
        const roman = size(12);
  ...
  describe("the no-drift guard", () => {
    ...
    it("a tune with no %%vocalfont draws lyrics at exactly the default size", () => {
      ...
      for (const mode of [
        ...
        expect(score.voices[0]?.measures[0]?.events[0]?.type === "note").toBe(
          true,
        );
      }
    });
  ...
  describe("I: is %% by another name (ABC 2.1 §11.4)", () => {
    ...
    it("does not break the +: chain the way the generic field dispatch did", () => {
      ...
      expect(evs.map((e) => (e.type === "rest" ? null : e.lyric))).toEqual([
        "la",
        "la",
        "la",
        "la",
