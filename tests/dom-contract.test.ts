...
function contractOf(svg: string): Row[] {
  ...
  for (const m of svg.matchAll(/<(\/?)([a-zA-Z]+)([^>]*?)(\/?)>/g)) {
    ...
    if (tag === "svg" || tag === "style" || tag === "title" || tag === "defs") {
      ...
      else if (closing === "" && selfClose === "" && tag !== "svg")
        stack.push(false);
      continue;
    }
    if (closing === "/") {
    ...
    if (counts)
      rows.push({ depth: depth(), class: klass === "" ? null : klass, name });
...
const show = (r: Row | undefined): string =>
  r === undefined
    ? "nothing"
    : `d${r.depth} class=${JSON.stringify(r.class)} name=${JSON.stringify(r.name)}`;
...
function firstDifference(
  ...
  if (got.length > want.length) {
    return {
      matched: want.length,
      where: `${got.length} rows, want ${want.length}: extra ${show(got[want.length])}`,
    };
  }
  return null;
}
...
function run(c: Case): Diff | null {
  return firstDifference(
    contractOf(renderAll(c.abc, { add_classes: true })[0]?.svg ?? ""),
    c.contract,
  );
}
  ...
  it("writes the ranked table", () => {
    ...
    const text = [
      ...
      ...off
        ...
        .map(
          (r) =>
            `  ${r.slug.padEnd(22)} ${String(r.diff?.matched).padStart(3)}/${r.n} ok  ${r.diff?.where}`,
        ),
    ].join("\n");
