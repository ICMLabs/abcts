...
export function absolutePixels(svg: string): PixelDoc {
  ...
  const vb = /viewBox="([-\d.]+)\s+([-\d.]+)\s+([-\d.]+)\s+([-\d.]+)"/.exec(
    svg,
  );
  const W = dim?.[1] !== undefined ? +dim[1] : null;
  const H = dim?.[2] !== undefined ? +dim[2] : null;
  const [vx = 0, vy = 0, vw = W ?? 1, vh = H ?? 1] = vb
    ? vb.slice(1).map(Number)
    : [];
  ...
  for (const m of svg.matchAll(tagRe)) {
    ...
    if (inDefs) continue;
    const tr = /transform="translate\(\s*([-\d.]+)\s*,\s*([-\d.]+)\s*\)/.exec(
      attrs,
    );
    ...
    const here =
      tr?.[1] !== undefined && tr[2] !== undefined
        ? { x: top.x + +tr[1], y: top.y + +tr[2] }
        : top;
    if (tag === "g" && !selfClose) {
