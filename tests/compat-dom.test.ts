...
it('emits every abcjs class the golden SVGs contain', () => {
  ...
  for (const file of readdirSync(corpusDir).filter((f) => f.endsWith('.abc'))) {
    ...
    for (const tune of renderAbc(null, readFileSync(join(corpusDir, file), 'utf-8'), {
      staffwidth: 740,
    })) {
  ...
  expect(compared, 'no golden SVG matched a corpus fixture').toBeGreaterThan(20)
  expect(theirs.size, 'no abcjs classes found in the goldens').toBeGreaterThan(3)
