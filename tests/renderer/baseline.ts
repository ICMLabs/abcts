...
function snapshotTune(score: Score, index: number): string {
  ...
  doc.systems.forEach((system, index) => {
    lines.push(
      `system ${index} width=${n(system.width)} originY=${n(system.originY)}`,
    );
    for (const g of system.connectorGlyphs) {
      const scale =
        g.scale === undefined || g.scale === 1 ? "" : ` scale=${n(g.scale)}`;
    ...
    system.staves.forEach((staff, staffIndex) => {
      ...
      for (const l of staff.voltaLines) {
        lines.push(
          `    volta  ${n(l.x1)},${n(l.y1)} -> ${n(l.x2)},${n(l.y2)} t=${n(l.thickness)}`,
        );
      }
      for (const t of staff.voltaTexts) {
        lines.push(
          `    voltatext ${n(t.x)},${n(t.y)} ${JSON.stringify(t.text)}`,
        );
      }
      ...
      for (const t of staff.tupletTexts) {
        lines.push(
          `    tuplettext ${n(t.x)},${n(t.y)} size=${n(t.size)} ${JSON.stringify(t.text)}`,
        );
      }
      ...
      for (const l of staff.melismaLines) {
        lines.push(
          `    melisma ${n(l.x1)},${n(l.y1)} -> ${n(l.x2)},${n(l.y2)} t=${n(l.thickness)}`,
        );
      }
      for (const l of staff.spannerLines) {
        lines.push(
          `    spanner ${n(l.x1)},${n(l.y1)} -> ${n(l.x2)},${n(l.y2)} t=${n(l.thickness)}`,
        );
      }
      ...
      for (const el of staff.elements) {
        const steps =
          el.staffSteps.length > 0 ? `@${el.staffSteps.join(",")}` : "";
