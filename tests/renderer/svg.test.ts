...
const svgFor = (abc: string): string =>
  toSVG(layout(parse(abc).scores[0] as Score));
...
describe("output shape", () => {
  ...
  it("puts the whole drawing inside the viewBox", () => {
    ...
    const offsetOf = (chunk: string) =>
      Number(
        /^[^>]*transform="translate\(0,(-?[\d.]+)\)"/.exec(chunk)?.[1] ?? "NaN",
      );
    ...
    for (const system of systems) {
      ...
      for (const staff of system
        .split('<g class="abcts-staff-group"')
        .slice(1)) {
        const staffY = offsetOf(staff);
        expect(Number.isFinite(staffY)).toBe(true);
        for (const m of staff.matchAll(
          /<rect[^>]*y="(-?[\d.]+)"[^>]*height="([\d.]+)"/g,
        )) {
          ...
          expect(bottom).toBeLessThanOrEqual(
            (minY as number) + (height as number),
          );
          checked++;
        }
      }
...
describe("%%jazzchords", () => {
  ...
  it("nests the modifier and the bass as small tspans", () => {
    const texts = chordsOf(
      'X:1\n%%jazzchords\nK:C\n"C7"C "C/B"B "x"A "x/C"G "/E"E|\n',
    );
    expect(texts[0]).toContain(
      '>C<tspan dy="-0.3em" style="font-size:0.7em">7</tspan>',
    );
    // A bass with no modifier before it drops 0.1em, not 0.4em.
    expect(texts[1]).toContain(
      '>C<tspan dy="0.1em" style="font-size:0.7em">/B</tspan>',
    );
    expect(texts[3]).toContain(
      '<tspan dy="0.4em" style="font-size:0.7em">/C</tspan>',
    );
    expect(texts[4]).toContain(
      '<tspan dy="0.1em" style="font-size:0.7em">/E</tspan>',
    );
  });
  ...
  it("reserves a whole line of height per nested tspan", () => {
    ...
    const heightOf = (abc: string) =>
      layout(parse(abc).scores[0] as Score).height;
