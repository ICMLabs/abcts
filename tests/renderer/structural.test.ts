...
const describeGolden = (el: GoldenLayoutElement): string => {
  ...
  const steps = (el.heads ?? [])
    .map((h) => h.pitch - ABCJS_MIDDLE_LINE)
    .sort((a, b) => a - b);
...
function coreSequence(abc: string): string[] {
  ...
  return layout(score).systems.flatMap((system, systemIndex) =>
    (system.staves[0]?.voices[0] ?? [])
      ...
      .map((el) =>
        el.staffSteps.length === 0
          ? el.type
          : `${el.type}@${el.staffSteps.join(",")}`,
      ),
  );
}

const goldenSequence = (name: string): string[] =>
  goldenLayoutElements(name).map(describeGolden);
...
describe("structural render parity vs abcjs layout", () => {
  ...
  const { cases: corpus, skipped: ungoldened } = withGolden(
    all,
    ".elements.json",
  );
  ...
  describe("the gate itself", () => {
    ...
    it("compares every notehead of a chord, not just the lowest", () => {
      ...
      expect(
        goldenSequence("ragtime-mini").some((s) => /^note@-?\d+,/.test(s)),
      ).toBe(true);
    });
  ...
  describe("renderable", () => {
    for (const name of RENDERABLE) {
      it(`${name} — element sequence and staff positions match`, () => {
        ...
        expect(
          fixture,
          `${name} is in RENDERABLE but not in the corpus`,
        ).toBeDefined();
        ...
        expect(
          golden.length,
          `${name} has an empty layout golden`,
        ).toBeGreaterThan(0);
  ...
  it("records its coverage for the parity tracker", () => {
    ...
    expect(RENDERABLE.length + Object.keys(KNOWN_DIVERGENCES).length).toBe(
      corpus.length,
    );
  });
  ...
  describe("coverage is accounted for", () => {
    const notYet = corpus.filter(
      (c) =>
        !RENDERABLE.includes(c.name) && KNOWN_DIVERGENCES[c.name] === undefined,
    );
    ...
    it("RENDERABLE and KNOWN_DIVERGENCES do not overlap or name absent fixtures", () => {
      ...
      for (const name of [...RENDERABLE, ...Object.keys(KNOWN_DIVERGENCES)]) {
        expect(names.has(name), `${name} is listed but not in the corpus`).toBe(
          true,
        );
      }
      for (const name of RENDERABLE) {
        expect(
          ...
          `${name} is both renderable and divergent`,
        ).toBeUndefined();
      }
    });
    ...
    for (const fixture of notYet) {
      it(`${fixture.name} — still diverges, so absent from RENDERABLE`, () => {
        ...
        expect(
          golden.length,
          `${fixture.name} has an empty layout golden`,
        ).toBeGreaterThan(0);
        expect(
          core,
          `${fixture.name} now matches abcjs layout — add it to RENDERABLE`,
        ).not.toEqual(golden);
      });
    }
  });
