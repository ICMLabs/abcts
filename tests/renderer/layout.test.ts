...
describe("accidentalGlyph", () => {
  ...
  it("maps the remaining accidentals", () => {
    ...
    expect(accidentalGlyph(Accidental.doubleSharp)).toBe(
  ...
  it("prints the natural end to end, in a key where it changes the pitch", () => {
    ...
    expect(notes[0]?.glyphs.map((g) => g.name)).toEqual([
      "accidentalNatural",
      "noteheadBlack",
    ]);
...
describe("clefs", () => {
  ...
  it("reads the octave suffix", () => {
    expect(parse("X:1\nK:C treble-8\nC|\n").scores[0]?.clef.octaveShift).toBe(
      -1,
    );
    expect(parse("X:1\nK:C treble+8\nC|\n").scores[0]?.clef.octaveShift).toBe(
      1,
  ...
  it("draws the staff lines `stafflines=` asks for, abcjs’s way", () => {
    ...
    const rules = (abc: string): number[] =>
      (
        layout(parse(abc).scores[0] as Score).systems[0]?.staves[0]
          ?.staffLines ?? []
      ).map((l) => l.y1);
...
describe("tempo", () => {
  ...
  it("reads every spelling the corpus uses", () => {
    expect(tempoOf("Q:1/4=120")).toEqual({
      beatUnit: rational(1, 4),
      bpm: 120,
      text: null,
      postText: null,
  ...
  it("gives a bare rate the METER’s beat unit, not the L: one", () => {
    ...
    const withMeter = (m: string, q: string) =>
      parse(`X:1\nM:${m}\nL:1/4\n${q}\nK:C\nC|\n`).scores[0]?.tempo ?? null;
    expect(withMeter("6/8", "Q:120")).toEqual({
      beatUnit: rational(1, 8),
      bpm: 120,
      text: null,
      postText: null,
    });
...
describe("chords", () => {
  ...
  const notesOf = (abc: string) =>
    elementsOf(abc).filter((e) => e.type === "note");
  ...
  it("draws one notehead per pitch", () => {
    ...
    expect(
      chord?.glyphs.filter((g) => g.name === "noteheadBlack"),
    ).toHaveLength(3);
  });
  ...
  it("moves a second across the stem instead of overlapping two heads", () => {
    ...
    const chord = notesOf("X:1\nL:1/4\nK:C\n[CDE]|\n")[0];
    const xs = (chord?.glyphs ?? [])
      .filter((g) => g.name === "noteheadBlack")
      .map((g) => g.x);
...
describe("flags and beams", () => {
  const sys = (abc: string) =>
    layout(parse(abc).scores[0] as Score).systems[0]?.staves[0];
  const notesOf = (abc: string) =>
    (sys(abc)?.elements ?? []).filter((e) => e.type === "note");
  const glyphNames = (abc: string) =>
    notesOf(abc).flatMap((n) => n.glyphs.map((g) => g.name));
  const stemOf = (el: {
    lines: readonly {
      x1: number;
      x2: number;
  ...
  it("lands every stem on the beam", () => {
    ...
    for (const note of (system?.elements ?? []).filter(
      (e) => e.type === "note",
    )) {
      ...
      const edge =
        (stem?.x1 ?? 0) + ((stem?.thickness ?? 0) / 2) * (up ? 1 : -1);
      expect(Math.abs((stem?.y2 ?? 0) - (yAt(edge) - half))).toBeLessThan(
        0.001,
      );
    }
  });
  ...
  it("clamps the slope so a beam stays gently inclined", () => {
    ...
    expect(Math.abs((beam?.y2 ?? 0) - (beam?.y1 ?? 0))).toBeLessThanOrEqual(
      2.001 * SPACE,
    );
  });
  ...
  it("draws a second beam only where consecutive notes both carry one", () => {
    ...
    const span = (b?: { x1: number; x2: number }) =>
      (b?.x2 ?? 0) - (b?.x1 ?? 0);
...
describe("system breaking", () => {
  ...
  it("does NOT re-wrap when the page is narrow — it compresses", () => {
    ...
    const wide = layout(parse(long(40)).scores[0] as Score, {
      systemWidth: 200,
    });
    const narrow = layout(parse(long(40)).scores[0] as Score, {
      systemWidth: 40,
    });
  ...
  it("reprints the clef and key on every system, but the meter and tempo only once", () => {
    ...
    doc.systems.forEach((system, index) => {
      ...
      if (index === 0) {
        ...
        expect(
          types,
          `system ${index} must not reprint the meter`,
        ).not.toContain("timeSignature");
        expect(
          types,
          `system ${index} must not reprint the tempo`,
        ).not.toContain("tempo");
      }
    });
  });
  ...
  it("always places at least one measure, even one wider than the page", () => {
    ...
    expect(
      doc.systems[0]?.staves[0]?.elements.some((e) => e.type === "note"),
    ).toBe(true);
  });
...
`;
  ...
  it("gives each staff its own clef, so the same step is a different pitch", () => {
    ...
    const clefOf = (i: number) =>
      doc.systems[0]?.staves[i]?.elements.find((e) => e.type === "clef")
        ?.glyphs[0]?.name;
    ...
    const firstNote = (i: number) =>
      doc.systems[0]?.staves[i]?.elements.find((e) => e.type === "note")
        ?.staffSteps[0];
  ...
  it("prints the tempo once, on the top staff — it belongs to the tune", () => {
    ...
    const tempos = doc.systems.flatMap((s, i) =>
      s.staves.map((st, j) => ({
        i,
        j,
        n: st.elements.filter((e) => e.type === "tempo").length,
      })),
    );
  ...
  it("aligns measures across staves, so bar 2 starts at the same x on both", () => {
    ...
    const barsOf = (i: number) =>
      (doc.systems[0]?.staves[i]?.elements ?? [])
        .filter((e) => e.type === "bar")
        .map((e) => e.x);
  ...
  it("stacks staves within a system without overlapping", () => {
    ...
    for (let i = 1; i < staves.length; i++) {
      expect(staves[i]?.originY ?? 0).toBeGreaterThan(
        staves[i - 1]?.originY ?? 0,
      );
    }
  });
...
describe("spacing and justification", () => {
  ...
  it("follows a square-root curve, so a note four times as long is twice as wide", () => {
    ...
    expect(
      naturalWidth(rational(1, 2), std) / naturalWidth(rational(1, 8), std),
    ).toBeCloseTo(2, 5);
  });

  it("never lets a note fall below the rod floor", () => {
    expect(
      naturalWidth(rational(1, 1024), ENGRAVE.spacingScale),
    ).toBeGreaterThanOrEqual(0.6 * SPACE);
    expect(
      naturalWidth(rational(0, 1), ENGRAVE.spacingScale),
  ...
  it("keeps ink from being crushed by a short duration", () => {
    ...
    const widthOf = (s: typeof bare) =>
      s?.elements.find((e) => e.type === "note")?.width ?? 0;
  ...
  it("justifies every system to the same width", () => {
    ...
    for (const w of doc.systems.map((s) => s.width))
      expect(w).toBeCloseTo(target, 5);
  });
  ...
  it("leaves a system short rather than stretching it absurdly", () => {
    ...
    for (const system of doc.systems)
      expect(system.width).toBeLessThanOrEqual(90 * SPACE);
  });
});
...
describe("barline shapes", () => {
  ...
  const shape = (bar: {
    lines: readonly { thickness: number }[];
  ...
  it("straddles the middle line with the repeat dots", () => {
    ...
    expect(dots.map((g) => g.name)).toEqual([
      "augmentationDot",
      "augmentationDot",
    ]);
...
describe("slurs and ties", () => {
  ...
  it("nests slurs, matching each close to the most recent open", () => {
    ...
    const curves = [...curvesOf("(G(GG)G)|")].sort(
  ...
  it("keeps the arc shallow however long the span", () => {
    ...
    expect(Math.abs(curvesOf("(GG)|")[0]?.bulge ?? 0)).toBeGreaterThanOrEqual(
      0.5 * SPACE,
    );
  });
...
describe("grace notes, chord symbols, lyrics and decorations", () => {
  ...
  it("draws grace notes small, before the note they lead into", () => {
    ...
    const graces = (note?.glyphs ?? []).filter(
      (g) => g.scale !== undefined && g.scale < 1,
    );
  ...
  it("slashes an acciaccatura with a GLYPH, and not an appoggiatura at all", () => {
    const slashes = (abc: string) =>
      (notesOf(abc)[0]?.glyphs ?? []).filter(
        (g) => g.name === "graceNoteSlashStemUp",
      ).length;
  ...
  it("places an articulation away from the stem", () => {
    ...
    const low = notesOf(".C|")[0]?.glyphs.find((g) =>
      g.name.startsWith("articStaccato"),
    );
    const high = notesOf(".c'|")[0]?.glyphs.find((g) =>
      g.name.startsWith("articStaccato"),
    );
  ...
  it("maps the decorations it knows and draws nothing for the rest", () => {
    ...
    const named = (abc: string) =>
      (notesOf(abc)[0]?.glyphs ?? [])
        .map((g) => g.name)
        .filter((n) => !n.startsWith("notehead"));
  ...
  it("stacks several decorations on one note without overlapping", () => {
    ...
    const glyphs = (notesOf(".!accent!!tenuto!G|")[0]?.glyphs ?? []).filter(
      (g) => g.name.startsWith("artic"),
    );
...
describe("tunebooks", () => {
  ...
  it("lays each tune out independently of the ones above it", () => {
    ...
    const second = layoutBook(
      parse(`${book(1)}X:9\nT:Solo\nL:1/4\nK:C\nCDEF|\n`).scores,
    );
    const strip = (doc: ReturnType<typeof layoutBook>, i: number) =>
      (doc.systems[i]?.staves[0]?.elements ?? []).map(
        (e) => `${e.type}@${e.x.toFixed(3)}`,
      );
  ...
  it("heads each tune with its title, inside the layout so it is not clipped", () => {
    ...
    doc.systems.forEach((system, i) => {
      ...
      const top =
        system.originY + (system.staves[0]?.originY ?? 0) + (text?.y ?? 0);
      expect(
...
describe("noteGlyph", () => {
  ...
  it("splits a dotted duration into its base note and dot count", () => {
    ...
    expect(noteGlyph(rational(3, 8))).toMatchObject({
      head: "noteheadBlack",
      dots: 1,
      flags: 0,
    });
    expect(noteGlyph(rational(3, 4))).toMatchObject({
      head: "noteheadHalf",
      dots: 1,
    });
    expect(noteGlyph(rational(3, 16))).toMatchObject({
      head: "noteheadBlack",
      dots: 1,
      flags: 1,
    });
    expect(noteGlyph(rational(7, 16))).toMatchObject({
      head: "noteheadBlack",
      dots: 2,
    });
    expect(noteGlyph(rational(15, 16))).toMatchObject({
      head: "noteheadHalf",
      dots: 3,
    });
    expect(noteGlyph(rational(3, 2))).toMatchObject({
      head: "noteheadWhole",
      dots: 1,
    });
  });
  ...
  it("tells a breve from a dotted note by the numerator's ODD part", () => {
    ...
    expect(noteGlyph(rational(2, 1))).toMatchObject({
      head: "noteheadDoubleWhole",
      dots: 0,
    });
    expect(noteGlyph(rational(3, 1))).toMatchObject({
      head: "noteheadDoubleWhole",
      dots: 1,
    });
  ...
  it("never sees a tuplet ratio, because notatedDuration excludes it by contract", () => {
    ...
    const score = parse("X:1\nM:4/4\nL:1/8\nK:C\n(3abc|\n").scores[0];
    const notes = (score?.voices[0]?.measures[0]?.events ?? []).filter(
      (e) => e.type === "note",
    );
    expect(notes).toHaveLength(3);
    for (const note of notes) {
      expect(
        note.type === "note" && noteGlyph(note.notatedDuration),
      ).toMatchObject({
        head: "noteheadBlack",
      });
    }
...
describe("compatibility modes", () => {
  ...
  it("renders at abcjs density in strict mode, and core's own otherwise", () => {
    const gap = (mode: CompatibilityMode) => {
      const abc = "X:1\nM:4/4\nL:1/4\nK:C\nCDEF|\n";
      const notes = (
        layout(parse(abc, { mode }).scores[0] as Score, { mode }).systems[0]
          ?.staves[0]?.elements ?? []
      ).filter((e) => e.type === "note");
...
describe("tuplets", () => {
  ...
  it("numbers a tuplet, so a triplet is not three plain notes", () => {
    ...
    expect(staffOf("(5cdefg c2|")?.tupletTexts.map((t) => t.text)).toEqual([
      "5",
    ]);
  ...
  it("clears the stems and beams, not just the noteheads", () => {
    ...
    expect(text?.y ?? 0).toBeGreaterThan(
      Math.max(beam?.y1 ?? 0, beam?.y2 ?? 0),
    );
  });
});
...
describe("repeat endings (voltas)", () => {
  ...
  it("leaves the shared barline between two endings", () => {
    ...
    expect((rules[1]?.x1 ?? 0) - (rules[0]?.x2 ?? 0)).toBeCloseTo(
      4 / UNIT_PX,
      3,
    );
  });
...
describe("annotations", () => {
  ...
  it("reserves `width + 7` before a `<` and 4 either side of a `>`", () => {
    ...
    expect(noteX('"<F"F G|') - noteX("F G|")).toBeCloseTo(
      (9.7813 + 7) / UNIT_PX,
      3,
    );
...
describe("melisma extenders", () => {
  ...
  it("keeps strict's underscore when the hold wraps to the next system", () => {
    ...
    const narrow = layout(
      parse(abc, { mode: "abcjs-strict" }).scores[0] as Score,
      {
        mode: "abcjs-strict",
        systemWidth: 26,
      },
    ...
    const texts = narrow.systems.flatMap((s) =>
      s.staves.flatMap((st) =>
        st.elements.flatMap((e) => e.texts.map((t) => t.text)),
      ),
    );
...
describe("styled noteheads", () => {
  ...
  it("lets the DURATION still pick filled vs open", () => {
    ...
    ).toEqual(["noteheadDiamondBlack", "noteheadDiamondWhite"]);
  });
...
describe("microtonal accidentals", () => {
  const accidentalsOf = (abc: string, mode: CompatibilityMode) =>
    (
      layout(parse(abc, { mode }).scores[0] as Score, {
        mode,
        systemWidth: 300,
      }).systems[0]?.staves[0]?.elements ?? []
    )
...
describe("flags beyond the sixteenth", () => {
  ...
  it("leaves beamed runs to the beam pass, which already had the levels right", () => {
    ...
    expect(
      staff?.elements.flatMap((e) => e.glyphs).filter((g) => g.role === "flag"),
    ).toEqual([]);
    // Three beam levels for a 32nd, which is what a flag would have carried.
    expect(new Set((staff?.beams ?? []).map((b) => b.y1.toFixed(3))).size).toBe(
      3,
    );
  });
});
...
describe("spanning decorations", () => {
  const staffOf = (abc: string, width = 300) =>
    layout(parse(abc).scores[0] as Score, { systemWidth: width }).systems[0]
      ?.staves[0];
  ...
  it("keeps ONE open hairpin per kind, and closes it at the line end", () => {
    ...
    const strokes = doc.systems
      .flatMap((sys) => sys.staves)
      .flatMap((st) => st.spannerLines);
...
describe("ornaments and techniques", () => {
  ...
  it("charges an ending's extra minspacing to ONE voice, not to every barline", () => {
    ...
    expect(firstNoteAfterEnding(withAcc)).toBeCloseTo(
      firstNoteAfterEnding(plain),
      6,
    );
  });
  ...
  it("draws `!slide!` as a CURVE at the note, and reserves nothing above", () => {
    ...
    if (curve === undefined || head === undefined)
...
describe("navigation directions", () => {
  ...
  it("draws them as WORDS, in ABCJS'S OWN SPELLING", () => {
    ...
    expect(textsOf("!D.S.alcoda!").map((t) => t.text)).toEqual([
      "D.S. al coda",
    ]);
    expect(textsOf("!D.C.alfine!").map((t) => t.text)).toEqual([
      "D.C. al fine",
    ]);
  });
...
describe("text metrics", () => {
  ...
  it("measures narrow and wide characters differently", () => {
    ...
    expect(leftOf("WWWWW", "abcjs-strict")).toBeGreaterThan(
      leftOf("iiiii", "abcjs-strict"),
    );
  });
  ...
  it("falls back for a character the table lacks, rather than measuring zero", () => {
    ...
    expect(leftOf("ЖЖЖ") - leftOf("Ж")).toBeCloseTo(
      CHAR_ADVANCE_BOLD_FALLBACK * sizeOf("Ж"),
      4,
    );
  });
...
describe("%%score / %%staves staff grouping", () => {
  const stavesFor = (abc: string) =>
    layout(parse(abc).scores[0] as Score, { systemWidth: 300 }).systems[0]
      ?.staves.length ?? 0;
  ...
  it("keeps the voices distinguishable on a shared staff", () => {
    ...
    const staff = layout(
      parse(THREE_VOICES("%%score (1 2 3)")).scores[0] as Score,
      {
        systemWidth: 300,
      },
    ).systems[0]?.staves[0];
    expect(staff?.voices).toHaveLength(3);
    expect(staff?.elements.length).toBe(
      (staff?.voices ?? []).reduce((n, v) => n + v.length, 0),
    );
  });
...
describe("stem direction on a shared staff", () => {
  const stemsPerVoice = (abc: string) => {
    const staff = layout(parse(abc).scores[0] as Score, { systemWidth: 300 })
      .systems[0]?.staves[0];
...
describe("staff connectors — braces and brackets", () => {
  ...
  it("braces a `{ … }` group and brackets a `[ … ]` one", () => {
    ...
    expect(
      systemFor("%%score {1 2}")?.connectorGlyphs.map((g) => g.name),
    ).toEqual(["brace"]);
    expect(
      systemFor("%%score [1 2]")?.connectorGlyphs.map((g) => g.name),
    ).toEqual(["bracketTop", "bracketBottom"]);
  });
...
describe("drawing bounds include prose, not just music", () => {
  const doc = (abc: string) =>
    layout(parse(abc).scores[0] as Score, { systemWidth: 20 });
