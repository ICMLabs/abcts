...
describe('spacing and justification', () => {
  ...
  it('never lets a note fall below the rod floor', () => {
    expect(naturalWidth(rational(1, 1024), ENGRAVE.spacingScale)).toBeGreaterThanOrEqual(
      0.6 * SPACE,
    )
...
describe('compatibility modes', () => {
  ...
  it('renders at abcjs density in BOTH modes', () => {
    ...
    // measured identically in the goldens.
    //
    // ⚠️ **THIS ROW USED TO ASSERT 6.5 FOR EXTENDED** — core's own abcm2ps-calibrated
    // density, about 16% looser. Under the owner's byte-compatibility rule (2026-09-07) a
    // spacing constant is a LOOK, not a bug abcjs has, so the density is abcjs's in both
    // modes and `RenderProfile` is a caller override rather than something the mode reaches
    // for. Measured: this one constant was part of the 331 fixtures the layout `strict`
    // flag was splitting. See `tests/mode-bytes.test.ts`.
    expect(gap('abcjs-strict')).toBeCloseTo(5.474 * SPACE, 2)
    expect(gap('abcjs-extended')).toBeCloseTo(5.474 * SPACE, 2)
  })
...
describe('tremolo, phrases and technique', () => {
  ...
  it('stacks one grace flag per stroke in strict, and picks a tremolo bar otherwise', () => {
    ...
    const extended = (
      layout(parse('X:1\nL:1/4\nK:C\n!//!F|\n', { mode: 'abcjs-extended' }).scores[0] as Score, {
        mode: 'abcjs-extended',
        systemWidth: 200,
      }).systems[0]?.staves[0]?.elements ?? []
    )
...
describe('text metrics', () => {
  ...
  /**
   * ⚠️ **TWO ROWS WERE REMOVED HERE WITH THE DIVERGENCE THEY ASSERTED** (2026-09-07):
   * "treats CJK as full width" and "falls back for a character the table lacks". Both
   * measured `abcjs-extended` against the REAL per-em tables, where strict uses
   * `dump-svg.js`'s five ASCII tables and a flat 8. That correction showed up HEADLESS
   * ONLY — with a DOM both modes ask it (`text-measure.ts`) — and cost 156 of 691 fixtures
   * against the owner's byte-compatibility rule, so both modes now measure with the golden
   * tables. `realTextWidth` and `CHAR_ADVANCE_BOLD_FALLBACK` are unreachable in
   * consequence; see `layout.ts`'s `STRICT_TEXT_METRICS`.
   *
   * The row below kept its content and lost the word `strict` from its name: it is now the
   * rule for BOTH modes, which is exactly the point.
   */
  it("measures an unlisted character as the golden's flat 8, in both modes", () => {
    ...
    for (const mode of ['abcjs-strict', 'abcjs-extended'] as const) {
      const two = leftOf('ЖЖЖ', mode) - leftOf('Ж', mode)
      expect(two, mode).toBeCloseTo((2 * 8) / 2 / UNIT_PX, 4)
    }
  })
})
...
describe('drawing bounds include prose, not just music', () => {
  ...
  /**
   * ⚠️ **"fits every text it draws" WAS REMOVED HERE** (2026-09-07). It asserted that
   * nothing the renderer places falls outside the box it reports, and it ran in
   * `abcjs-extended` on purpose: strict measures with the golden generator's `calcWidth` —
   * a 27px title measured with 17px Times widths — so its declared box UNDER-reports BY
   * DESIGN, and abcjs's own SVG has the same property.
   *
   * Under the owner's byte-compatibility rule both modes now measure with those tables, so
   * there is no mode left in which the invariant holds. It is abcjs's behaviour rather than
   * a defect of ours, and asserting it of a mode that no longer differs would only fail.
   * See `layout.ts`'s `STRICT_TEXT_METRICS` and `tests/mode-bytes.test.ts`.
   */
})
