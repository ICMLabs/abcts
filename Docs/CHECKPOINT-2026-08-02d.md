...
## THE ABCJS TEST-SUITE CORPUS — harvested, gated, and what it found

`tests/corpus-abcjs/` holds **174 tunes** taken from abcjs's own `tests/`, with goldens
generated by running abcjs itself. `npm run harvest` rebuilds the fixtures,
`npm run harvest:goldens` the goldens, and `SOURCES.json` maps each one back to the test
file it came from. They live in abcts because `../abcMusicKit` is read-only from here; the
directory carries abcjs's MIT notice.

**The ASSERTIONS are not ported and are not worth porting.** abcjs's 272 `it()` cases read
its internal `visualObj` tree — `visualObj[0].lines[0].staff[0].voices[0][i].duration` —
which `abcts/compat` does not reproduce, and `tests/api/` `require()`s abcjs's own modules
directly. What transfers is the maintainers' choice of what is worth exercising.

The gate asserts three things and is deliberately NOT a table of 174 ceilings: that
nothing throws (**174/174**, the number to keep flat), that notehead counts match except
for a named list of gaps — a ratchet in both directions, so a fixed gap has to be struck
off — and how many fixtures land within 0.05 / 1 / 5 / 25px, as counts that only go up.
Standing at **72 / 82 / 90 / 116** of 172 content-correct fixtures.

### What it found that the 41-fixture corpus cannot see

| gap | state |
|---|---|
| **`&` OVERLAY VOICES parsed and never drawn** | FIXED |
| Zero-length notes (`C0`) dropped | FIXED — abcjs draws a stemless quarter head |
| `%%stretchlast` (13 fixtures), `%%staffwidth` (3) | FIXED |
| `%%maxStaves` incipits | FIXED |
| A grace note before a `y` SPACER | open — **a model decision**, below |
| A header block before the first `X:` splits the book | open |

**The overlay one is the lesson.** A whole feature was parsed correctly, stored correctly
and never painted, and 505 tests went green over it — because the corpus that gates
everything has no `&` in it anywhere. A GATE IS ONLY AS BROAD AS ITS INPUTS, and ours had
all been chosen by the same people who wrote the engine.

### The grace-before-spacer gap is a decision, not a fix

`(f3 {a})y` — the grace attaches to the `y` that follows it, and `Rest` has no
`graceNotes` field. The model says so on purpose: *"a rest does carry decorations — but
not ties, slurs, grace notes or lyrics, none of which apply to silence"*. abcjs disagrees
for a SPACER, which is a layout device rather than silence, and draws the grace with its
own stem, flag and ledger. Overturning a stated model decision needs a decision, so it is
recorded rather than taken.

### Still unimplemented, ranked by how many fixtures want them
