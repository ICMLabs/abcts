...
/**
 * ⚠️ **A PATH THAT CAN NEVER BE DRAWN IS NOT SHIPPED — 91 OF THE 119.**
 *
 * `glyph-table.ts`'s ABCJS table answers with abcjs's own outline wherever abcjs HAS the
 * shape, and falls through to Bravura's only where it does not. Since the owner's
 * byte-compatibility ruling of 2026-09-07 that table serves BOTH modes, so for every mapped
 * glyph Bravura's `path` is unreachable — while its METRICS stay very much alive: a
 * notehead's anchors are read straight off `GLYPHS` (`layout.ts`, "anchors are a property
 * of the shape"), and `bravuraDeclared` reads its height.
 *
 * So the METRICS of all 119 are kept and the PATH of the 91 the abcjs table wins is
 * omitted. **77 KB of the 122 KB of Bravura path data**, about 17 KB brotli, on a bundle
 * that was 52 KB bigger than abcjs's.
 *
 * ⚠️ **THE SET IS COMPUTED, NEVER A HAND-WRITTEN LIST**, so it cannot rot: it is read out
 * of `glyph-map.ts` and `glyphs-abcjs.ts` at generation time, and
 * `tests/glyph-map.test.ts` asserts the invariant from the other side — every glyph that
 * CAN be drawn has a path, and every stripped one is covered by abcjs's table. Change the
 * mapping and re-run this script; nothing here needs editing.
 *
 * To get them all back: delete the `strip` filter below and re-run. Bravura.otf is the
 * source and it has not moved.
 */
const mapSrc = readFileSync(resolve(root, 'src/renderer/glyph-map.ts'), 'utf-8')
const abcjsSrc = readFileSync(resolve(root, 'src/renderer/glyphs-abcjs.ts'), 'utf-8')
const mapped = new Map(
  [...mapSrc.matchAll(/^  ([A-Za-z][A-Za-z0-9]*): '([^']+)',/gm)].map((m) => [m[1], m[2]]),
)
const abcjsHas = new Set(
  [...abcjsSrc.matchAll(/^  '?([A-Za-z0-9_.]+)'?: \{/gm)].map((m) => m[1]),
)
/** True when the ABCJS table answers for this name, so Bravura's outline can never draw. */
const strip = (name) => {
  const to = mapped.get(name)
  return to !== undefined && abcjsHas.has(to)
}
const stripped = entries.filter((e) => strip(e.name)).length
console.error(
  `gen-glyphs: ${entries.length} glyphs, ${stripped} paths omitted (abcjs's table wins)`,
)
